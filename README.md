...
It is necessary to isolate the contribution of the magneto-optically active nanoparticles to the ellipsometry signal and eliminate the effects of the substrate/layer. Therefore, the change in the ellipsometry output angle psi with varying magnetic field is used to calculate Faraday rotation. The equation for angle of Faraday rotation is Φ = (ψB+ -  ψB0) · sgn(P) = (ψB0 - ψB+) · sgn(P) = (ψB+ -  ψB-)/2 · sgn(P) and consists of three equivalent expressions [1]. The first and second methods consider the difference between psi with the magnetic flux density direction parallel (B+) and anti-parallel (B-) to the direction of light and psi without an applied magnetic field (B0). The third method averages the first two and is therefore considered a best estimate of Faraday rotation. The factor sgn(P) is the sign of the angle of the polarizer associated with the corresponding psi measurements.
